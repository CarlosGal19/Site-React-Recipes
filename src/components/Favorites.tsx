...
const Favorites = () => {
  ...
  return (
    <>
      <h1 className='text-6xl font-bold text-center py-8 bg-gray-300 dark:bg-slate-700 dark:text-slate-100'>Favorites</h1>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 bg-gray-300 dark:bg-slate-700'>
        ...
        {favorite.length === 0 ? (
          ...
          favorite.map((item) => (
            <div key={item.id} className='shadow-md rounded p-4 text-center bg-indigo-200 dark:bg-zinc-700 mb-8'>
              <h2 className='font-bold text-3xl dark:text-slate-100'>{item.name}</h2>
